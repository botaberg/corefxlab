...
namespace System.Text.Primitives.Tests
{
    public partial class PrimitiveParserPerfComparisonTests
    {
        ...
        private static void BaselineUInt32(string text)
        {
        ...
        [Benchmark]
        private static void BaselineUInt32_VariableLength()
        {
            foreach (var iteration in Benchmark.Iterations)
            {
                uint sum = 0;
        ...
        [InlineData("4294967295")] // max value
        [InlineData("0")] // min value
        private unsafe static void InternalParserUInt32(string text)
        {
        ...
        [Benchmark]
        private unsafe static void InternalParserUInt32_VariableLength()
        {
        ...
        [InlineData("4294967295")] // max value
        [InlineData("0")] // min value
        private unsafe static void PrimitiveParserUInt32(string text)
        {
            int length = text.Length;
            byte[] utf8ByteArray = Encoding.UTF8.GetBytes(text);
            foreach (var iteration in Benchmark.Iterations)
        ...
        [Benchmark]
        private unsafe static void PrimitiveParserUInt32_VariableLength()
        {
            List<byte[]> byteArrayList = new List<byte[]>();
            foreach (string text in textArray)
            {
